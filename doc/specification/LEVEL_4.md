...
All `WalletService` operations must be **thread-safe**. Multiple concurrent calls to the service's methods (e.g., `TRANSFER`, `PAYMENT`, `DEPOSIT`) must not result in race conditions or inconsistent states.
...
- **Behavior of Existing Commands for Locked Accounts:**
    - `PAYMENT`, `TRANSFER`, `PAYMENT_WITH_CASHBACK`: Must fail (return `false` or `null`) if the source account is locked.
...
- **New Command: `SET_DAILY_LIMIT <accountId> <limit>`**
    ...
    - **Failure:** Returns `false` if the account does not exist.
...
- **Behavior of Debit Commands:**
    ...
    - If the limit is exceeded, the operation must fail (return `false` or `null`).
